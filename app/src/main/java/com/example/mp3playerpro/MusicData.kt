...
class MusicData(id: String, title: String?, artist: String?, albumId: String?, duration: Long?): Serializable {
    ...
    // 음악 앨범을 Uri를 통해서 Bitmap 가져오는 함수
    fun getAlbumBitmap(context: Context, albumImageSize: Int): Bitmap? {
        ...
        try {
            if (albumUri != null) {
                ...
                // 비트맵 사이즈를 결정
                if (bitmap != null) {
                    // 화면에 보여줄 이미지 사이즈가 맞지 않을 경우 강제로 사이즈 조절
    ...
    // 이전 음악 앨범을 가져오는 Uri 함수
    fun getPreviousMusicUri(): Uri = Uri.withAppendedPath(MediaStore.Audio.Media.EXTERNAL_CONTENT_URI, this.id.toMutableList()-1)
}